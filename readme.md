...
#### Player Photos
There is currently no toggle for this, to disable it you would have to edit the `focusedplayer.vue` file. To add player photos you should drag the image into the `players` folder located in the `img` folder in `src`. These images must be in .svg (you can convert them at https://theonlineconverter.com/convert-png-to-svg) and are case sensitive.
